...
class on_file_all_files_to_parent_folder:
    ...
    def __call__(self, *args, **kwargs) -> None:
        ...
        self.__call__.add_line(funcs_file.file_all_to_parent_folder(folder_path))
...
class on_file_block_disks:
    ...
    def __call__(self, *args, **kwargs) -> None:
        ...
        output = funcs_dev.dev_run_powershell_script_as_admin(commands)
...
class on_file_check_featured_image:
    ...
    def __call__(self, *args, **kwargs) -> None:
        ...
        try:
            self.__call__.add_line(funcs_file.file_check_featured_image(folder_path)[1])
...
class on_file_check_featured_image_in_folders:
    ...
    def __call__(self, *args, **kwargs) -> None:
        ...
        for path in paths_with_featured_image:
            try:
                self.__call__.add_line(funcs_file.file_check_featured_image(path)[1])
            except Exception as e:
                self.__call__.add_line(f"❌ Error: {e}")
...
class on_file_open_camera_uploads:
    ...
    def __call__(self, *args, **kwargs) -> None:
        ...
        funcs_file.file_open_file_or_folder(folder_path)
        funcs_file.file_open_file_or_folder(folder_path / "info")
        funcs_file.file_open_file_or_folder(folder_path / "temp")
        funcs_file.file_open_file_or_folder(folder_path / "video")
        funcs_file.file_open_file_or_folder(folder_path / "work")
        funcs_file.file_open_file_or_folder(folder_path / "work_video")
        funcs_file.file_open_file_or_folder(folder_path / "screenshots")
...
class on_file_tree_view_folder:
    ...
    def __call__(self, *args, **kwargs) -> None:
        ...
        result_output = funcs_file.file_tree_view_folder(folder_path, kwargs.get("is_ignore_hidden_folders", False))
