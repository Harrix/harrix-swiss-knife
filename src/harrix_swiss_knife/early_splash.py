...
def _apply_window_geometry(user32: ctypes.WinDLL, hwnd: int) -> None:
...
def _paint(hwnd: int) -> None:
    ...
    try:
        ...
        key_brush = gdi32.CreateSolidBrush(_KEY_COLOR)
        if key_brush:
        ...
        old_brush = gdi32.SelectObject(hdc, fill) if fill else 0
        old_pen = gdi32.SelectObject(hdc, pen) if pen else 0
        ...
        band = max(1, height // 3)
        _draw_line(user32, gdi32, hdc, title, _rect(0, band // 8, width, band), title_font)
        _draw_line(user32, gdi32, hdc, status, _rect(0, band, width, band), body_font)
        _draw_line(user32, gdi32, hdc, clock, _rect(0, band * 2 - band // 8, width, band), body_font)
        if title_font:
...
def _register_class(user32: ctypes.WinDLL, instance: int) -> None:
    ...
    class_info.style = _CS_HREDRAW | _CS_VREDRAW
    ...
    class_info.hbrBackground = _COLOR_WINDOW + 1
...
def _run_message_loop() -> None:
    ...
    if not hwnd:
        return
    user32.SetLayeredWindowAttributes(hwnd, _KEY_COLOR, 0, _LWA_COLORKEY)
    with _lock:
        _runtime.hwnd = hwnd
    ...
    while not _runtime.stop.is_set() and _pump_once(user32, msg):
        _apply_window_geometry(user32, hwnd)
        user32.InvalidateRect(hwnd, None, True)  # noqa: FBT003
        time.sleep(_POLL_S)
    with contextlib.suppress(OSError):
