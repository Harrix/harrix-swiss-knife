"""Shared VS Code style settings editor for `config.json`.

The development Settings action opens this dialog with every key. Each desktop
app opens the same dialog filtered to keys that belong to that app.
...
SHARED_APPS_SETTING_KEYS = frozenset({"initial_count", "load_more_count", "local_language"})
TRACKER_APP_IDS = frozenset({"finance", "fitness", "food", "habits"})
_APP_EXTRA_KEY_PREFIXES: dict[str, tuple[str, ...]] = {
    "habits": ("habit_", "path_habit_"),
    "icons": ("path_vector_icons", "vector_icons_"),
}
...
class SettingsEditorDialog(QDialog):
    ...
    def __init__(
        ...
        *,
        ...
        - `app_id` (`str | None`): When set, show only keys for this app
          (`fitness`, `food`, `finance`, `habits`, `icons`). Defaults to all keys.
        - `window_title` (`str | None`): Dialog title. Defaults to `Settings`.

        """
        super().__init__(parent)
        self._app_id = app_id
        self.setWindowTitle(window_title or "Settings")
        ...
        self._full_config = copy.deepcopy(self.config_data)
    ...
    def _persist_config(self) -> bool:
        self._save_current_category()
        assembled = assemble_config(self.categories, self._key_order)
        new_config = merge_filtered_config(self._full_config, assembled) if self._app_id else assembled
...
def config_key_belongs_to_app(key: str, app_id: str) -> bool:
    """Return whether a top-level or nested setting key belongs to `app_id`."""
    key_norm = key.lower().replace("-", "_")
    app_norm = app_id.lower().replace("-", "_")
    if key_norm in {app_norm, f"sqlite_{app_norm}"} or key_norm.startswith(f"{app_norm}_"):
        return True
    return any(key_norm.startswith(prefix) for prefix in _APP_EXTRA_KEY_PREFIXES.get(app_norm, ()))


def filter_config_categories(categories: dict[str, dict[str, Any]], app_id: str) -> dict[str, dict[str, Any]]:
    """Keep only settings that belong to `app_id`, preserving category order."""
    result: dict[str, dict[str, Any]] = {}
    for cat_name, settings in categories.items():
        if cat_name == "General":
            filtered = {key: value for key, value in settings.items() if config_key_belongs_to_app(key, app_id)}
        elif config_key_belongs_to_app(cat_name, app_id):
            filtered = settings
        else:
            filtered = {
                key: value for key, value in settings.items() if nested_setting_belongs_to_app(cat_name, key, app_id)
            }
        if filtered:
...
def merge_filtered_config(full_config: dict[str, Any], assembled: dict[str, Any]) -> dict[str, Any]:
    """Write filtered editor values back into `full_config` without dropping other keys."""
    result = copy.deepcopy(full_config)
...
def nested_setting_belongs_to_app(category: str, key: str, app_id: str) -> bool:
    """Return whether a key inside a nested config object belongs to `app_id`."""
