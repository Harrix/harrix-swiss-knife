...
class AutoSaveOperations:
    ...
    def _save_process_habbits_data(
        ...
        try:
            if not value_str or value_str.strip() == "":
                ...
                if record_id is not None:
                    ...
                    # Clear stored data
                    item.setData(
                        (None, habbit_id, date_str), Qt.ItemDataRole.UserRole
                    )
                return
        ...
        if not self._is_valid_date(date_str):
            QMessageBox.warning(
                None, "Validation Error", "Use YYYY-MM-DD date format"
            )
            return
        ...
        if record_id is not None:
            ...
            if not self.db_manager.update_process_habbit_record(
                record_id, habbit_id, value, date_str
            ):
                QMessageBox.warning(
                    None,
                    "Database Error",
            ...
            if existing_records and len(existing_records) > 0:
                ...
                if not self.db_manager.update_process_habbit_record(
                    existing_record_id, habbit_id, value, date_str
                ):
                    QMessageBox.warning(
                        None,
                        "Database Error",
            ...
            # Create new record
            elif self.db_manager.add_process_habbit_record(
                habbit_id, value, date_str
            ):
