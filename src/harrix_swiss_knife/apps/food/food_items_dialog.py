...
class FoodItemsDialog(QDialog):
    ...
    def _show_table_context_menu(self, position: QPoint) -> None:
        ...
        if food_item_id is not None:
            ...
            edit_action.triggered.connect(lambda: self._open_edit_dialog(cast("int", food_item_id)))
        ...
        if food_item_id is not None:
            delete_action.triggered.connect(lambda: self._on_delete(cast("int", food_item_id)))
