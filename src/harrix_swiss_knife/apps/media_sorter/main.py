"""Media Sorter GUI — sort photos/videos into config-defined bins."""
...
from PySide6.QtGui import QCloseEvent, QDesktopServices, QDrag, QIcon, QKeyEvent, QMouseEvent, QPixmap
...
from harrix_swiss_knife.apps.media_sorter import database_manager, window
from harrix_swiss_knife.apps.media_sorter.database_manager import normalize_media_path
from harrix_swiss_knife.apps.media_sorter.media_scan import (
    is_media_path,
    is_video_path,
    iter_media_files,
...
from harrix_swiss_knife.apps.media_sorter.sort_ops import (
    BinConfig,
    assign_to_bin,
    parse_bins_from_config,
    resolve_working_path,
    trash_file,
)
from harrix_swiss_knife.apps.media_sorter.thumbnails import load_media_thumbnail
...
_BIN_DROP_STYLE = (
    ...
    " min-height: 72px;"
    "}"
    "QFrame#binDropFrame:hover { border-color: #4a90d9; }"
)
...
class _DraggablePreviewLabel(QLabel):
    """Preview label that starts a file drag when the user drags the pixmap."""
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:  # noqa: N802
        if (
            ...
            if pixmap is not None and not pixmap.isNull():
                drag.setPixmap(pixmap.scaled(96, 96, Qt.AspectRatioMode.KeepAspectRatio))
    ...
    def set_drag_path(self, path: str | None) -> None:
        """Set the file path used for drag-and-drop."""
