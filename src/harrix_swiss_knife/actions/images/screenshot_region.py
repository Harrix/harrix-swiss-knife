...
class OnScreenshotRegion(ActionBase):
    ...
    Starts in region-selection mode with the app hidden. A leftover modal
    result dialog does not turn keep-Windows on. The left shutter column can
    arrange the desktop, keep app Windows visible (re-grabs), skip the preview
    (clipboard only), adjust the frame, or show guides; close cancels.
