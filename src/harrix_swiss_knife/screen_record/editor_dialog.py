"""Editor window for a finished screen recording (trim, play, export)."""
...
_FORMAT_FILTERS = {
    ...
    "avif": "AVIF animation (*.avif)",
    "avif_optimized": "AVIF optimized (*.avif)",
}
...
def _format_ms(ms: int) -> str:
    ...
    frac = max(0, ms) % 1000
    if hours:
        return f"{hours:d}:{minutes:02d}:{seconds:02d}.{frac:03d}"
    return f"{minutes:d}:{seconds:02d}.{frac:03d}"
