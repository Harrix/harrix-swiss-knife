...
class GalleryCleanerPreferences(
    ...
    /** Records a successful delete toward all-time Gallery Cleaner totals. */
    fun recordDeletedPhoto(sizeBytes: Long) {
        adjustLifetimeDeleteStats(deletedDelta = 1, freedBytesDelta = sizeBytes)
    }

    /** Reverses [recordDeletedPhoto] when a trashed photo is restored (undo). */
    fun recordRestoredDeletedPhoto(sizeBytes: Long) {
        adjustLifetimeDeleteStats(deletedDelta = -1, freedBytesDelta = -sizeBytes)
    }
