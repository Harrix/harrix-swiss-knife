...
Fenced code blocks in the built-in **Markdown preview** (including notes opened via **Harrix Notes** with `openNotesInPreview`) show a **Copy** button on hover. Preview scripts run only in a **trusted** workspace; if the button is missing, check workspace trust and **Markdown: Preview Security Settings**.
