...
- Do **not** fill `Score` or `Date` (and similar date fields like `Date watching`) from a screenshot — leave them omitted unless the value is clearly stated in the user text.
...
- For coordinate fields (`Coordinates`, `Coords`, `LatLon`, and similar), prefer values from the browser address bar / map URL when present (e.g. `ll=`, `@lat,lon`, `mlat`/`mlon`, `q=lat,lon`). Coordinates are often available there even when not shown as text on the map screenshot.
