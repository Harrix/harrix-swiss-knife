...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "function Copy-Deps([string]$srcDeps,[string]$dstDeps,[string[]]$excludeDirs,[string[]]$excludeFiles){ if(-not (Test-Path -LiteralPath $srcDeps)){ throw ('Not found: ' + $srcDeps) }; New-Item -ItemType Directory -Path $dstDeps -Force | Out-Null; Get-ChildItem -LiteralPath $srcDeps -Force | ForEach-Object { if($_.PSIsContainer){ if($excludeDirs -contains $_.Name){ return }; Copy-Item -LiteralPath $_.FullName -Destination (Join-Path $dstDeps $_.Name) -Recurse -Force } else { if($_.Name -like '*.log'){ return }; if($excludeFiles -and ($excludeFiles -contains $_.Name)){ return }; Copy-Item -LiteralPath $_.FullName -Destination (Join-Path $dstDeps $_.Name) -Force } } } " ^
  "function Zip-Dir([string]$dir,[string]$zip){ if(Test-Path -LiteralPath $zip){ Remove-Item -LiteralPath $zip -Force -ErrorAction SilentlyContinue }; [System.IO.Compression.ZipFile]::CreateFromDirectory($dir, $zip, [System.IO.Compression.CompressionLevel]::Optimal, $false); if(-not (Test-Path -LiteralPath $zip)){ throw ('Zip was not created: ' + $zip) } } " ^
  ...
  "  Write-Host ('Created: ' + $outOnline) -ForegroundColor Green; " ^
  "  Write-Host ('Created: ' + $outOffline) -ForegroundColor Green; " ^
