...
$log = Join-Path $PSScriptRoot "install-all-offline.log"
if (-not (Test-Path -LiteralPath $script)) {
    Write-Error "Not found: $script"
    exit 1
}

$runner = Join-Path $env:TEMP ("harrix-swiss-knife-install-offline-{0}.ps1" -f ([guid]::NewGuid().ToString("N")))
...
$runnerContent = @(
    ...
    '    # Offline-first: prefer offline installers/repos in install\dependencies;',
    '    # -SkipBinaries still copies ffmpeg/avif from the bundle, but skips network downloads.',
    '    & $ScriptPath -SkipBinaries -NoPauseOnError -UseOfflineRepoSnapshots',
