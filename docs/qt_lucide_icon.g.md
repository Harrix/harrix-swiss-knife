...
## 🔧 Function `create_tabler_icon`

```python
def create_tabler_icon(name: str, size: int = DEFAULT_LUCIDE_MENU_ICON_SIZE, *, color: QColor | str | None = None, device_pixel_ratio: float | None = None) -> QIcon
```
...
- [`size`](apps/icons/lightbox_cache.g.md#%EF%B8%8F-method-size-property) (`int`): Logical side length in pixels.
