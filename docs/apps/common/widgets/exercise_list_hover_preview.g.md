...
- [🏛️ Class `ExerciseListHoverPreview`](#%EF%B8%8F-class-exerciselisthoverpreview)
  ...
  - [⚙️ Method `add_view`](#%EF%B8%8F-method-add_view)
...
```python
def __init__(self, list_view: QAbstractItemView, *, get_avif_manager: Callable[[], AvifManager | None], preview_size: QSize | None = None, parent: QObject | None = None, exercise_at: Callable[[QPoint], str | None] | None = None) -> None
```
...
```python
def add_view(self, view: QAbstractItemView, exercise_at: Callable[[QPoint], str | None]) -> None
```
...
## 🔧 Function `exercise_at_table_image`

```python
def exercise_at_table_image(table_view: QAbstractItemView, pos: QPoint, *, image_column: int = 0, name_column: int = 1) -> str | None
```
