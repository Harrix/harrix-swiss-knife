...
Preview + metadata form for one new note-icon.
...
```python
def __init__(self, parent: QWidget | None, *, source_path: Path, defaults: RepoMetaDefaults, app_config: dict[str, Any]) -> None
```
