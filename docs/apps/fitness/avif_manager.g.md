...
Args: - `avif_dir` (`Path | str`): Directory path containing AVIF files.
...
Args: - `label_key` (`str`): Key identifying which label.

Returns: - `str | None`: Current exercise name or None.
...
Args: - `exercise_name` (`str`): Name of the exercise.

Returns: - `Path | None`: Path to the AVIF file if it exists, None otherwise.
...
Args: - `avif_path` (`Path`): Path to the AVIF file.

Returns: - `QPixmap | None`: Loaded pixmap or None if loading failed.
...
Args: - `exercise_name` (`str`): Name of the exercise to load AVIF for. - `label_widget` (`QLabel`): Label widget to display the AVIF. - `label_key` (`str`): Key identifying which label to update
('main', 'exercises', 'types', 'charts', 'statistics'). Defaults to `"main"`.
...
Args: - `label_key` (`str`): Key identifying which label to update.
