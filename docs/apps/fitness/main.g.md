...
- [🏛️ Class `MainWindow`](#%EF%B8%8F-class-mainwindow)
  ...
  - [⚙️ Method `on_edit_dumbbell_weights`](#%EF%B8%8F-method-on_edit_dumbbell_weights)
  ...
  - [⚙️ Method `on_sync_dumbbell_weight_types`](#%EF%B8%8F-method-on_sync_dumbbell_weight_types)
...
- [🔧 Function `enqueue_deferred_exercise_icon`](#-function-enqueue_deferred_exercise_icon)
- [🔧 Function `exercise_names_from_name_column`](#-function-exercise_names_from_name_column)
- [🔧 Function `exercise_table_focus_column`](#-function-exercise_table_focus_column)
- [🔧 Function `is_exercise_table_image_column`](#-function-is_exercise_table_image_column)
- [🔧 Function `prioritize_deferred_exercise_icons`](#-function-prioritize_deferred_exercise_icons)
...
class MainWindow(
    ...
    def _check_for_monthly_goal_achievement(self, ex_id: int, added_value: float, date_str: str) -> tuple[bool, float]:
        ...
        try:
            # Get exercise name
            exercise_rows = self.db_manager.get_rows(
                "SELECT name FROM exercises WHERE _id = :id",
                {"id": ex_id},
            )
            if exercise_rows:
                exercise_name = exercise_rows[0][0]
                months_count = self.spinBox_compare_last.value()
    ...
    def _copy_template_weight_types(
        self,
        exercise_id: int,
        template: list[WeightTypeSpec],
        *,
        require_type: bool,
    ) -> int:
        """Add missing template weight types to one exercise.

        Args:

        - `exercise_id` (`int`): Target exercise ID.
        - `template` (`list[WeightTypeSpec]`): Types to copy.
        - `require_type` (`bool`): Whether to set `is_type_required`.

        Returns:

        - `int`: Number of types that were inserted.

        """
        if self.db_manager is None:
            return 0
        existing_names = [spec.name for spec in self.db_manager.get_exercise_weight_type_specs(exercise_id)]
        added = 0
        for spec in missing_weight_types(existing_names, template):
            if self.db_manager.add_exercise_type(
                exercise_id,
                spec.name,
                spec.calories_modifier,
                name_local=spec.name_local,
                ...
                logger.error("Failed to add type %s to exercise %s", spec.name, exercise_id)
    ...
    def _get_dumbbell_weight_template_specs(self) -> list[WeightTypeSpec] | None:
        ...
        - `list[WeightTypeSpec] | None`: Template types, or `None` when they cannot be loaded.
        ...
        if not specs:
            message_box.warning(
                self,
                "Dumbbell Weight Types",
                f"Template exercise '{DUMBBELL_WEIGHT_TEMPLATE_EXERCISE}' has no weight types.",
            )
            return None
        return specs
    ...
    def _get_last_weight(self) -> float:
        ...
        try:
            ...
            logger.exception("Error getting last weight")
            return initial_weight
        else:
    ...
    def _load_exercise_avif(self, exercise_name: str, label_key: str = "main") -> None:
        ...
        if not label_widget:
            logger.info("%s", f"Unknown label key: {label_key}")
            return
    ...
    def _show_monthly_goal_congratulations(self, exercise: str, type_name: str, current_value: float) -> None:
        ...
        try:
            ...
            logger.exception("Error showing monthly goal congratulations")
    ...
    def _show_record_congratulations(self, exercise: str, record_info: dict) -> None:
        ...
        try:
            ...
            logger.exception("Error showing record congratulations")
    ...
    def _update_form_from_process_selection(self, _exercise_name: str, type_name: str, value_str: str) -> None:
        ...
        try:
            ...
            logger.exception("Error updating form from process selection")
...
### ⚙️ Method `on_edit_dumbbell_weights`
...
### ⚙️ Method `on_sync_dumbbell_weight_types`
...
## 🔧 Function `enqueue_deferred_exercise_icon`
...
## 🔧 Function `exercise_names_from_name_column`
...
## 🔧 Function `exercise_table_focus_column`
...
## 🔧 Function `is_exercise_table_image_column`
...
## 🔧 Function `prioritize_deferred_exercise_icons`

```python
def prioritize_deferred_exercise_icons(pending: OrderedDict[str, None], priority_names: Sequence[str]) -> None
```
