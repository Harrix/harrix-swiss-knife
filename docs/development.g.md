...
Full `hsk` CLI catalog (icons + descriptions): [`README.md` → CLI commands](https://github.com/Harrix/harrix-swiss-knife/blob/main/README.md#-cli-commands).
...
The editor **Preview** tab in **Cursor** and current **VS Code** is Tiptap/ProseMirror and still loads `markdown.markdownItPlugins`. This extension does **not** inject extra HTML there (`harrixNotesExplorerHsk.previewMarkdownItExtensions` stays `false`) — doing so blanks the pane. Use **Markdown: Open Preview** / **Open Preview to the Side** (`Ctrl+Shift+V`) for the classic webview with copy buttons and YAML collapsed into `📋 YAML`.
