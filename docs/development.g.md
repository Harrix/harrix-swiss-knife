...
In **Cursor**, the editor **Preview** tab (next to Source) is a Tiptap/ProseMirror surface, not the VS Code webview. This extension does **not** wrap that renderer with extra document HTML (doing so blanks the pane). YAML there is a plain table when the host emits a `front_matter` token. Use **Markdown: Open Preview** / **Open Preview to the Side** (`Ctrl+Shift+V`) for the classic webview with copy buttons and YAML collapsed into `📋 YAML`. VS Code's built-in `yamlPreamble` table is wrapped the same way.
