...
def test_format_commit_message_for_book_uses_english_title_and_author() -> None:
    message = format_commit_message(
        '➕ Add book "{TitleEnglish}" {AuthorEnglish}',  # noqa: RUF001
        {
    ...
    assert message == '➕ Add book "The Chain of Chance" Stanislaw Lem'  # noqa: RUF001
...
def test_build_commit_message_for_template_uses_config_pattern() -> None:
    message = build_commit_message_for_template(
        ...
        {"Title": "Трон: Арес", "Original or English title": "Tron: Ares"},  # noqa: RUF001
    )
    assert message == '🎬 Add movie "Tron: Ares"'


def test_build_commit_message_for_builtin_commands() -> None:
    assert build_commit_message_for_command("new_diary") == "➕ Add diary note"  # noqa: RUF001
    assert build_commit_message_for_command("new_cases") == "➕ Add cases"  # noqa: RUF001
    ...
    assert (
        ...
        == '➕ Add quotes from Author — "Book"'  # noqa: RUF001
    )
