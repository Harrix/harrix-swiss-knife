...
def test_filter_rows_by_name_searches_name_and_english() -> None:
    rows = [
        [1, "2026-01-01", 50, 200, "Ветчина Для тостов", "Ham toast", 0],
        [2, "2026-01-02", 200, 50, "Молоко 2.5%", "Milk", 1],
        [3, "2026-01-03", 85, 80, "Корм для кошек", "Tuna", 0],
    ]
...
def test_get_filtered_food_log_records_by_type_and_date(food_db: DatabaseManager) -> None:
    ...
    assert [row[4] for row in drinks] == ["Tea", "Milk"]
    ...
    assert [row[4] for row in january_food] == ["Bread"]
...
def test_get_filtered_food_log_records_by_name(food_db: DatabaseManager) -> None:
    ...
    assert [row[4] for row in rows] == ["Ветчина"]

    rows_en = food_db.get_filtered_food_log_records(name_filter="milk")
    assert [row[4] for row in rows_en] == ["Молоко"]
